...
import { psychologyUnits, PSYCH_REVIEW_SLUG, psychologyReviewUnit } from "@/data/psychology";
import { courseCovers } from "@/data/course-covers";

const FlashcardsPsych = () => {
  useTitle("AP Psychology Flashcards");

  return (
    <Layout wide>
      <section className="relative overflow-hidden">
        <img src={courseCovers["ap-psych"]} alt="" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-8 py-16 sm:py-24 space-y-6">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Vocab decks for each CED unit, plus a combined deck when you want the full course in one stack.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 sm:px-8 py-12 sm:py-20">
        <div className="max-w-5xl mx-auto space-y-4">
          {psychologyUnits.map((unit, i) => (
            ...
            >
              <Link
                to={`/units/ap-psych/flashcards/${unit.id}`}
              ...
              >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                    <Clock size={14} />
                    {unit.period} of the exam · {unit.flashcards.length} cards
                  </p>
                </div>
          ...
          >
            <Link
              to={`/units/ap-psych/flashcards/${PSYCH_REVIEW_SLUG}`}
              className="group flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 p-6 sm:p-8 rounded-[1.75rem] border border-primary/30 bg-primary/5 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all"
            >
              ...
              <div className="flex-1 min-w-0 space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {psychologyReviewUnit.flashcards.length} cards from all five units
                </p>
              </div>
              <span className="inline-flex items-center font-bold text-sm text-primary shrink-0">
                Open deck
          ...
          <div className="pt-6">
            <Button asChild variant="outline" className="rounded-xl h-12">
              <Link to="/units/ap-psych">Go to AP Psych library</Link>
            </Button>
          </div>
        </div>
      </section>
...
export default FlashcardsPsych;
